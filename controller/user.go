...
func UserInfo(c *gin.Context) {
	...
	//查询点赞数量和作品数量并更新
	_ = dao.DB.Model(&model.Video{}).Where("is_favorite = ? AND user_id = ?", true, UserId).Count(&total_favorited).Error
	_ = dao.DB.Model(&model.Video{}).Where("user_id = ?", UserId).Count(&work_count).Error
